...
async function getTopRepositories() {
    ...
    if (!response.ok) {
        throw new Error(`A requisição falhou com o código de status: ${response.status}`);
    }
    const allRepositories = await response.json();
    console.log();
    const sortRepositories = allRepositories.sort((a, b) => b.stargazers_count - a.stargazers_count);
